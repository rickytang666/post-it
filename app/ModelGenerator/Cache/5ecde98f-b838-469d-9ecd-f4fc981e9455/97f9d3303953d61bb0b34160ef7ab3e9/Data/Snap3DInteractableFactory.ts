...
export class Snap3DInteractableFactory extends BaseScriptComponent {
  ...
  @input
  buttonPrefab: ObjectPrefab; // prefab for the always-accessible sticky note button

  private avaliableToRequest: boolean = true;
  private wcfmp = WorldCameraFinderProvider.getInstance();

  onAwake() {
    // Create a persistent sticky note button floating in front of the user
    const buttonObj = this.buttonPrefab.instantiate(this.sceneObject);
    buttonObj.getTransform().setWorldPosition(this.wcfmp.getForwardPosition(50)); // 50 units in front of user
    buttonObj.getTransform().setWorldRotation(this.wcfmp.getForwardRotation()); // face the user

    buttonObj.createEvent("TapEvent").bind(() => {
      this.createInteractable3DObject(this.prompt);
    });

    print("Sticky note button ready!");
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      this.generateSnap3DPrompt(originalText)
        .then((cleanPrompt) => {
          ...
          // Show original text on the object
