...
let InteractionPlane = class InteractionPlane extends BaseScriptComponent {
    ...
    buildMeshShape() {
        ...
        shape.size = new vec3(this.planeSize.x + DEFAULT_LERP_OFFSET_CM, this.planeSize.y + DEFAULT_LERP_OFFSET_CM, (this.proximityDistance + DEFAULT_LERP_OFFSET_CM) * 2);
    ...
    projectPoint(point) {
        ...
        const isWithinInteractionZone = distance >= 0 &&
            distance <= this.proximityDistance + DEFAULT_LERP_OFFSET_CM &&
            Math.abs(x) <= this.planeSize.x / 2 + DEFAULT_LERP_OFFSET_CM &&
            Math.abs(y) <= this.planeSize.y / 2 + DEFAULT_LERP_OFFSET_CM;
        ...
        const horizontalLerpValue = distanceToHorizontalEdge > 0
            ? 1 - Math.min(distanceToHorizontalEdge, DEFAULT_LERP_OFFSET_CM) / DEFAULT_LERP_OFFSET_CM
            : 1;
        const distanceToVerticalEdge = Math.abs(y) - this.planeSize.y / 2;
        const verticalLerpValue = distanceToVerticalEdge > 0
            ? 1 - Math.min(distanceToVerticalEdge, DEFAULT_LERP_OFFSET_CM) / DEFAULT_LERP_OFFSET_CM
            : 1;
        const distanceToProximityEdge = distance - this.proximityDistance;
        const proximityLerpValue = distanceToProximityEdge > 0
            ? 1 - Math.min(distanceToProximityEdge, DEFAULT_LERP_OFFSET_CM) / DEFAULT_LERP_OFFSET_CM
            : 1;
